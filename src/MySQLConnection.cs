...
namespace MySQLDriverCS
{
	...
	public class MySQLConnection : DbConnection
	{
		...
		internal unsafe void* handle = null;
		...
		/// </summary>
		public override unsafe ConnectionState State
		{
			get
			{
				if (handle == null)
		...
		public override unsafe void ChangeDatabase(string databaseName)
		{
			if (0 == NativeMethods.mysql_select_db(handle, databaseName))
				this.dbname = databaseName;
			else
			{
				throw new MySQLException("MySQLDriverCS Error: change database failed, perhaps user is not authorized to access that database." + NativeMethods.mysql_error(handle));
			}
		}
		#endregion
		...
		public override unsafe void Open()
		{
			...
			handle = NativeMethods.mysql_init(null);
			...
			// Timeout option
			NativeMethods.mysql_options(handle, 0, &timeout);

			// Compression option
			uint _null = 0;
			if (activate_compression)
				NativeMethods.mysql_options(handle, 0, &_null);

			// END ADDITION 2004-07-01

			void* retval = NativeMethods.mysql_real_connect(handle, location, userid, password, database, Convert.ToUInt32(port), null, 0);
			/* Explicit error conection:
			 * "Christophe Ravier" <c.ravier@laposte.net> 2003-11-27*/
			if (retval == null)
			{
				throw new MySQLException(NativeMethods.mysql_errno(handle), "MySQLDriverCS Error: can't connect." + NativeMethods.mysql_error(handle));
			}
			try
			{
				if (characterset != null && characterset != string.Empty)
				{
					if (NativeMethods.mysql_set_character_set(handle, characterset) == 0)
					{
			...
			catch (EntryPointNotFoundException)
			{
				...
				throw new MySQLException("MySQLDriverCS Error: 'Character Set' keyword not supported in client version " + version);
			}
		}
		#endregion
		#region Close
		...
		public override unsafe void Close()
		{
			...
			NativeMethods.mysql_close(handle);
			handle = null;
		}
		#endregion
	...
	public class MySQLException : System.Data.Common.DbException
	{
		...
		/// <param name="_message"></param>
		public MySQLException(uint _number, string _message)
		{
			this.message = _message;
			this.number = _number;
		}
		...
		{
			get { return "MySQLDriverCS Exception: " + message; }
		}
		/// <summary>
